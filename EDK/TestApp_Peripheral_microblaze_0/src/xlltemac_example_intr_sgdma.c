...
#define DMA_RX_IRPT_INTR  XPAR_XPS_INTC_0_PPC440_0_DMA0RXIRQ_INTR 
#define DMA_TX_IRPT_INTR  XPAR_XPS_INTC_0_PPC440_0_DMA0TXIRQ_INTR
#endif
...
int main(void)
{
	...
	/*
	 * Call the Temac SGDMA interrupt example , specify the parameters
			  ...
			  u16 TemacIntrId, u16 DmaRxIntrId, u16 DmaTxIntrId)
{
	 ...
	 */
	if (XLlTemac_GetPhysicalInterface(TemacInstancePtr) == XTE_PHY_TYPE_MII)
	{
		LoopbackSpeed = TEMAC_LOOPBACK_SPEED;
	} else {
		LoopbackSpeed = TEMAC_LOOPBACK_SPEED_1G;
	}
	TemacUtilEnterLoopback(TemacInstancePtr, LoopbackSpeed);

	/*
	 * Set PHY<-->MAC data clock
	 */
	XLlTemac_SetOperatingSpeed(TemacInstancePtr, (u16)LoopbackSpeed);
	...
	/*
	 * Current(11.1) hardware design will disable Tx checksum when any of
	 * Tx VLAN feature is built in hardware. Later releases will not have
	 * this limitations... Please update this if statement accordingly.
	...
	if (XLlTemac_IsRxCsum(TemacInstancePtr) &&
	    ...
	    !XLlTemac_IsTxVlanTran(TemacInstancePtr) &&
	    !XLlTemac_IsTxVlanStrp(TemacInstancePtr) &&
	    !XLlTemac_IsTxVlanTag(TemacInstancePtr)) {
				     ...
				     XLlDma * DmaInstancePtr)
{
	 ...
	 */
	if ((XLlDma_mBdRead(Bd1Ptr, XLLDMA_BD_USR4_OFFSET) & 0x3FFF)
		!= TxFrameLength) {
				     ...
				     XLlDma * DmaInstancePtr)
{
	 ...
	 * the real length of the received packet
	 */
	if ((XLlDma_mBdRead(BdPtr, XLLDMA_BD_USR4_OFFSET) & 0x3FFF)
		!= TxFrameLength) {
		TemacUtilErrorTrap("Length mismatch");
		return XST_FAILURE;
	}
