...
## 
## $Header: /devl/xcs/repo/env/Jobs/MDT/sw/Apps/debug/new_xmd/DataFiles/Attic/genace.tcl,v 1.1.2.13 2008/12/03 18:37:26 rajn Exp $
###############################################################################
...
#
# Note: 
# This script has been tested for "Supported" board listed below
...
# Production ML507
set ml507(jtag_chain_options) "-configdevice devicenr 1 idcode 0x032c6093 irlength 10 partname xc5vfx70t_u"
set ml507(jtag_fpga_position) 1
set ml507(jtag_devices) "xc5vfx70t_u"

# Production ML510
set ml510(jtag_chain_options) "-configdevice devicenr 1 idcode 0x03300093 irlength 14 partname xc5vfx130t"
set ml510(jtag_fpga_position) 1
set ml510(jtag_devices) "xc5vfx130t"
...
proc xmd_elf2svf { target_type elffile xmd_options } {
    ...
    if { $target_type == "ppc_hw" } {
	    if { [is_ppc405_target $tgt ] } {
          puts "Set CCR0 Value to 0x50700000"
			 set regIndex [lsearch $_ppc_reg_names ccr0]
			 set regIndex [expr $regIndex+31]
			 xwreg $tgt $regIndex 0x50700000 
		 }
    }
...
proc parse_genace_optfile { optfilename } {
	...
	switch -- $genace_opt {
		...
		while { $i < [llength $llist] } {
			...
			puts "jtag_fpga_pos is $jtag_fpga_position"
    ...
    if { [string match -nocase $param(board) "user"] && $configdevice } {
	error "\[-board user\] option missing -configdevice options"
    }
...
set board [string tolower $param(board)]
if {![string match $board "user"] && ![array exist $board] } {
    puts "ERROR: $board is not a valid board name.\n"
	 return
}
